...
const int pai_num = 120 ;				    		//5的全排列个数  5!=120; 6!=720; 7!=5040; 8!=40320; 9!=362880; 10!=3628800	——可设置
const int top_num = 5 ;							//前五个的排序									——可设置
const int jingdu = 1000 ;						//精度，100为2位，1000为3位，以此类推 			——可设置
const double epsilon = 0.1 ;					// ε											——可设置
...
void open()
{
	ar = fopen("dataset\\input_all-rank-of-5.txt","r");
	ss = fopen("dataset\\input_Jester-1-user_num-15000_item-5.txt","r");
	bordaxu = fopen ("dataset\\Output_Naive-RAPPOR_User'num-15000_item-5_Epsilon-0.1_for-1000_1.txt","a");	
}
