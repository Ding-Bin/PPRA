...
const int pai_num = 5040 ;				    		//5的全排列个数  5!=120; 6!=720; 7!=5040; 8!=40320; 9!=362880; 10!=3628800	——可设置
const int top_num = 7;							//前五个的排序									——可设置
const int jingdu = 1000 ;						//精度，100为2位，1000为3位，以此类推 			——可设置
const double epsilon = 0.05 ;					// ε											——可设置
...
void open()
{
	ar = fopen("dataset\\input_all-rank-of-7.txt","r");
	ss = fopen("dataset\\input_Jester-1-user_num-15000_item-7.txt","r");
	bordaxu = fopen ("dataset\\Output_Naive-RAPPOR_User'num-15000_item-7_Epsilon-0.05_for-1000_2.txt","a");	
}
