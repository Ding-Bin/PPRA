...
void Init()									
{
    voter_num = 15000 ;									//设置用户量 
    item_num = 50 ; 										//设置项目量 
	top_num = 50 ;										//设置top-k个排序量 
    for(i = 0 ; i < item_num ; i ++)
    {
        goal_item[i].item_xu = i ;			
    }
	fp = fopen("dataset\\input_Jester-1-user_num-15000_item-50.txt","r");
	p = fopen ("dataset\\Output_Borda's-Count-Kendall's-Tau_User'num-15000_item-50.txt","w");			
}
...
void Sort_item()
{
    sort(goal_item , goal_item + item_max , cmp) ;
}
